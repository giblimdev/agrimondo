...
import { Sun, Droplet, Leaf, Thermometer, TrendingUp, MapPin } from "lucide-react";

export default function CulturePage() {
  const [filter, setFilter] = useState<string>("all");
  const [sortBy, setSortBy] = useState<"note" | "order" | "name">("note");

  const filtered = filter === "all" ? cultures : cultures.filter(c => c.type === filter);
  ...
  return (
    <div className="p-6 space-y-6 bg-slate-50 min-h-screen">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold text-slate-900">Cultures - Popayán</h1>
      ...
      <ScrollArea className="h-[calc(100vh-250px)]">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-6">
          {sorted.map((culture: Culture) => (
            <Card key={culture.id} className="hover:shadow-xl transition-all duration-300 border-2">
              <CardHeader>
                <div className="flex justify-between items-start gap-2">
                  <div className="flex-1">
                    <CardTitle className="text-lg flex items-center gap-2 flex-wrap">
                      {culture.nomFrancais}
                      {culture.note && (
                        <Badge className={getNoteColor(culture.note)} variant="default">
                          {culture.note}/100
                        </Badge>
                      )}
                    </CardTitle>
              ...
              <CardContent className="space-y-3">
                ...
                <p className="text-sm text-slate-700 line-clamp-2">{culture.presentation}</p>
                ...
                {culture.rendement && culture.rendement.valeur && (
                  <div className="bg-slate-100 p-2 rounded text-xs">
                    <div className="flex items-center gap-1 font-semibold">
                      <TrendingUp size={14} />
                      Rendement: {culture.rendement.valeur.min}-{culture.rendement.valeur.max} {culture.rendement.valeur.unit}
                    </div>
                    {culture.cycle && (
                      <div className="text-slate-600 mt-1">
                        Cycle: {culture.cycle.dureeCycleJours.min}-{culture.cycle.dureeCycleJours.max} jours 
                        {culture.cycle.nbCyclesAn && (
                          <>({culture.cycle.nbCyclesAn.min}-{culture.cycle.nbCyclesAn.max} cycles/an)</>
                        )}
                      </div>
                    )}
                  </div>
                ...
                {culture.pertiance && culture.pertiance.length > 0 && (
                  <div className="bg-blue-50 border border-blue-200 p-2 rounded text-xs">
                    <p className="font-semibold text-blue-900 mb-1">Pertinence Popayán:</p>
